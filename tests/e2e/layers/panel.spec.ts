...
test('renaming focuses the input inside the flattened layer tree', async () => {
  // Regression for #288: Vue collected ref="renameInput" as an array
  // when the input rendered inside a flattened v-for, so .focus was
  // undefined and the user could not type into the rename field without
  // clicking it first.
  ...
  // The input must be focused on entry; typing should land in it
...
test('renaming a different layer transfers focus to the new input', async () => {
  // When rename is opened on one row and the user opens rename on
  // another, the input ref collection shifts. Vue 3 array refs are
  // updated reactively (stale refs removed, new refs appended), so the
  // newest input must end up focused, not the previously-removed one.

  const firstRow = layerRows().filter({ hasText: 'Components' }).first()
  await firstRow.dblclick()
  await editor.canvas.waitForRender()

  const firstInput = editor.page.getByTestId('layers-item-input')
  await expect(firstInput).toBeVisible()
  await firstInput.press('Escape')
  await editor.canvas.waitForRender()

  const otherRow = layerRows().nth(1)
  await otherRow.dblclick()
  await editor.canvas.waitForRender()

  const newInput = editor.page.getByTestId('layers-item-input')
  await expect(newInput).toBeVisible()
  const focused = await newInput.evaluate((el) => el === document.activeElement)
  expect(focused).toBe(true)

  await newInput.press('Escape')
  editor.canvas.assertNoErrors()
})
